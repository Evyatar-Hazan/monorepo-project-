...
export class LoansService {
  ...
  async findAllLoans(query: LoansQueryDto, user: any): Promise<LoansListResponseDto> {
  ...
  async updateLoan(id: string, updateLoanDto: UpdateLoanDto): Promise<LoanResponseDto> {
    ...
    try {
      ...
      if (status) {
        ...
        if (status === LoanStatus.RETURNED && existingLoan.status !== LoanStatus.RETURNED) {
  ...
  async markLoanAsLost(id: string, notes?: string): Promise<LoanResponseDto> {
    ...
    if (loan.status !== LoanStatus.ACTIVE && loan.status !== LoanStatus.OVERDUE) {
      throw new BadRequestException('רק השאלות פעילות או באיחור יכולות להיות מסומנות כאבודות');
    }
  ...
  async getLoanStats(): Promise<LoanStatsResponseDto> {
    ...
    const [totalActiveLoans, totalOverdueLoans, totalReturnedLoans, totalLostItems] =
      await Promise.all([
  ...
  private formatLoanResponse(loan: any): LoanResponseDto {
    ...
    const daysOverdue =
      isOverdue && loan.expectedReturnDate
        ? Math.floor(
            (now.getTime() - new Date(loan.expectedReturnDate).getTime()) / (1000 * 60 * 60 * 24)
          )
        : undefined;
